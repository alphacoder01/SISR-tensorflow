...
from utils import resolve, eval, vgg_22, vgg_54
class Trainer:
  def __init__(self, model, loss, learning_rate, checkpoint_dir= './ckpt/srgan'):
  ...
  def evaluate(self, dataset):
    return eval(self.checkpoint.model, dataset)
...
class SrganTrainer:
  def __init__(self,
    ...
    if content_loss == 'VGG22':
      ...
      raise ValueError("Content loss must be either VGG22 or VGG54")
  ...
  def train_step(self, lr, hr):
    ...
    gradients_of_gen = gen_tape.gradient(perc_loss, self.generator.trainable_variables)
    ...
    self.discriminator_optimizer.apply_gradients(zip(gradients_of_disc, self.discriminator.trainable_variables))
